    ...
    info = [info;['Über:' num2str((handles.daten.total_hydro(handles.config.mg,1)+sum(handles.daten.kep{handles.config.mg,handles.config.t}))/1000-handles.daten.load_balance(handles.config.mg,handles.config.t)/1000) ' GW']];
    end
    info = [info;['-----------------------------']];
